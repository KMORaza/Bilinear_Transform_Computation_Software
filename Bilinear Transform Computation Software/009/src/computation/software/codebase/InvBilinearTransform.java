...
public class InvBilinearTransform extends JFrame {
    ...
    private JTextArea poleZeroArea;
    private AnalogFrequencyResponsePanel freqResponsePanel;
    private PoleZeroPlotPanel poleZeroPlotPanel;
    ...
    public InvBilinearTransform(SymbolicTransferFunction tf, int precision, double T) {
        ...
        poleZeroArea = new JTextArea(6, 30);
        poleZeroArea.setEditable(false);
        poleZeroArea.setFont(new Font("Bahnschrift", Font.PLAIN, 12));
        poleZeroArea.setForeground(Color.WHITE);
        poleZeroArea.setBackground(new Color(50, 50, 50));
        JScrollPane poleZeroScroll = new JScrollPane(poleZeroArea);
        poleZeroScroll.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.LIGHT_GRAY), "Analog Poles and Zeros", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, bahnschriftFont, Color.WHITE));
        poleZeroScroll.setBackground(new Color(30, 30, 30));

        JSplitPane outputSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, outputScroll, poleZeroScroll);
        outputSplitPane.setDividerLocation(150);
        outputSplitPane.setBackground(new Color(30, 30, 30));
        ...
        poleZeroPlotPanel = new PoleZeroPlotPanel();
        poleZeroPlotPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.LIGHT_GRAY), "Analog Pole-Zero Plot", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, bahnschriftFont, Color.WHITE));

        JTabbedPane tabbedPane = new JTabbedPane();
        tabbedPane.setBackground(new Color(30, 30, 30));
        tabbedPane.setForeground(Color.WHITE);
        tabbedPane.setFont(bahnschriftFont);
        tabbedPane.addTab("Frequency Response", freqResponsePanel);
        tabbedPane.addTab("Pole-Zero Plot", poleZeroPlotPanel);

        add(controlPanel, BorderLayout.WEST);
        add(outputSplitPane, BorderLayout.SOUTH);
    ...
    private void computeInverseTransform() {
        try {
            ...
            // Compute and display analog poles and zeros
            computeAnalogPolesAndZeros();

        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Invalid sampling period format!", "Error", JOptionPane.ERROR_MESSAGE);
            outputArea.setText("Error: Invalid sampling period format!");
            poleZeroArea.setText("Error: Invalid sampling period format!");
        } catch (IllegalArgumentException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            outputArea.setText("Error: " + ex.getMessage());
            poleZeroArea.setText("Error: " + ex.getMessage());
        }
    }

    private void computeAnalogPolesAndZeros() {
        try {
            double[] numZ = discreteTf.getNumerator();
            double[] denZ = discreteTf.getDenominator();

            // Find roots of numerator (zeros) and denominator (poles)
            PolynomialRootFinder.Complex[] digitalZeros = PolynomialRootFinder.findRoots(numZ);
            PolynomialRootFinder.Complex[] digitalPoles = PolynomialRootFinder.findRoots(denZ);

            // Apply inverse bilinear transform: s_i = (2/T) * (z_i - 1)/(z_i + 1)
            ArrayList<PolynomialRootFinder.Complex> analogZeros = new ArrayList<>();
            ArrayList<PolynomialRootFinder.Complex> analogPoles = new ArrayList<>();
            ArrayList<String> analogZeroStrings = new ArrayList<>();
            ArrayList<String> analogPoleStrings = new ArrayList<>();
            ArrayList<String> warningMessages = new ArrayList<>();
            final double infinityThreshold = 1e-8;
            final double nearMinusOneThreshold = 1e-4;

            for (PolynomialRootFinder.Complex z : digitalZeros) {
                PolynomialRootFinder.Complex one = new PolynomialRootFinder.Complex(1.0, 0.0);
                PolynomialRootFinder.Complex zMinusOne = z.subtract(one);
                PolynomialRootFinder.Complex zPlusOne = z.add(one);
                double zPlusOneAbs = zPlusOne.abs();
                if (zPlusOneAbs < infinityThreshold) {
                    analogZeroStrings.add("infinity");
                } else {
                    PolynomialRootFinder.Complex s = zMinusOne.divide(zPlusOne).scale(2.0 / T);
                    analogZeros.add(s);
                    analogZeroStrings.add(formatComplex(s));
                    if (zPlusOneAbs < nearMinusOneThreshold) {
                        warningMessages.add(String.format("Warning: Digital zero near z = -1 (z = %.4f + j%.4f), maps to large s = %s", z.real, z.imag, formatComplex(s)));
                    }
                }
            }

            for (PolynomialRootFinder.Complex z : digitalPoles) {
                PolynomialRootFinder.Complex one = new PolynomialRootFinder.Complex(1.0, 0.0);
                PolynomialRootFinder.Complex zMinusOne = z.subtract(one);
                PolynomialRootFinder.Complex zPlusOne = z.add(one);
                double zPlusOneAbs = zPlusOne.abs();
                if (zPlusOneAbs < infinityThreshold) {
                    analogPoleStrings.add("infinity");
                } else {
                    PolynomialRootFinder.Complex s = zMinusOne.divide(zPlusOne).scale(2.0 / T);
                    analogPoles.add(s);
                    analogPoleStrings.add(formatComplex(s));
                    if (zPlusOneAbs < nearMinusOneThreshold) {
                        warningMessages.add(String.format("Warning: Digital pole near z = -1 (z = %.4f + j%.4f), maps to large s = %s", z.real, z.imag, formatComplex(s)));
                    }
                }
            }
            ...
            // Format output
            StringBuilder output = new StringBuilder();
            output.append("Analog Zeros:\n");
            if (analogZeroStrings.isEmpty()) {
                output.append("None\n");
            } else {
                for (String zero : analogZeroStrings) {
                    output.append(zero).append("\n");
                }
            }
            output.append("\nAnalog Poles:\n");
            if (analogPoleStrings.isEmpty()) {
                output.append("None\n");
            } else {
                for (String pole : analogPoleStrings) {
                    output.append(pole).append("\n");
                }
            }
            ...
            poleZeroArea.setText(output.toString());

            // Update pole-zero plot
            poleZeroPlotPanel.setPolesAndZeros(analogPoles, analogPoles, analogZeroStrings, analogPoleStrings);
            poleZeroPlotPanel.repaint();

        } catch (Exception e) {
            poleZeroArea.setText("Error computing analog poles and zeros: " + e.getMessage());
        }
    }

    private String formatComplex(PolynomialRootFinder.Complex c) {
        String format = "%." + precision + "f";
        if (Math.abs(c.imag) < 1e-8) {
            return String.format(format, c.real);
        } else if (Math.abs(c.real) < 1e-8) {
            return String.format(format + "j", c.imag);
        } else if (c.imag > 0) {
            return String.format(format + " + j" + format, c.real, c.imag);
        } else {
            return String.format(format + " - j" + format, c.real, -c.imag);
        }
    }
    ...
    private class PoleZeroPlotPanel extends JPanel {
        private ArrayList<PolynomialRootFinder.Complex> poles;
        private ArrayList<PolynomialRootFinder.Complex> zeros;
        private ArrayList<String> poleStrings;
        private ArrayList<String> zeroStrings;

        private PoleZeroPlotPanel() {
            super();
            setBackground(new Color(30, 30, 30));
            poles = new ArrayList<>();
            zeros = new ArrayList<>();
            poleStrings = new ArrayList<>();
            zeroStrings = new ArrayList<>();
        }

        public void setPolesAndZeros(ArrayList<PolynomialRootFinder.Complex> poles, ArrayList<PolynomialRootFinder.Complex> zeros, ArrayList<String> zeroStrings, ArrayList<String> poleStrings) {
            this.poles = poles;
            this.zeros = zeros;
            this.zeroStrings = zeroStrings;
            this.poleStrings = poleStrings;
            repaint();
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            double maxReal = 1.0;
            double maxImag = 1.0;
            for (PolynomialRootFinder.Complex z : zeros) {
                maxReal = Math.max(maxReal, Math.abs(z.real));
                maxImag = Math.max(maxImag, Math.abs(z.imag));
            }
            for (PolynomialRootFinder.Complex p : poles) {
                maxReal = Math.max(maxReal, Math.abs(p.real));
                maxImag = Math.max(maxImag, Math.abs(p.imag));
            }
            double maxAxis = Math.max(maxReal, maxImag);
            maxAxis = Math.max(1.0, maxAxis * 1.2); // 20% padding
            double minReal = -maxAxis;
            double maxRealAxis = maxAxis;
            double minImag = -maxAxis;
            double maxImagAxis = maxAxis;

            // Draw grid
            g2.setColor(new Color(50, 50, 50));
            int gridCount = 10;
            for (int i = 0; i <= gridCount; i++) {
                double real = minReal + i * (maxRealAxis - minReal) / gridCount;
                double imag = minImag + i * (maxImagAxis - minImag) / gridCount;
                int x = margin + (int) (i * (width - 2 * margin) / gridCount);
                int y = height - margin - (int) (i * (height - 2 * margin) / gridCount);
                g2.drawLine(x, margin, x, height - margin);
                g2.drawLine(margin, y, width - margin, y);
            }
            ...
            // Draw axes
            g2.setColor(Color.WHITE);
            int xZero = margin + (int) ((-minReal) * (width - 2 * margin) / (maxRealAxis - minReal));
            int yZero = height - margin - (int) ((-minImag) * (height - 2 * margin) / (maxImagAxis - minImag));
            g2.drawLine(xZero, margin, xZero, height - margin); // Imaginary axis
            g2.drawLine(margin, yZero, width - margin, yZero); // Real axis

            // Label axes
            g2.drawString("Re(s)", width - margin - 20, yZero + 15);
            g2.drawString("Im(s)", xZero - 30, margin + 15);
            String format = "%." + precision + "f";
            for (int i = 0; i <= 5; i++) {
                double real = minReal + i * (maxRealAxis - minReal) / 5;
                double imag = minImag + i * (maxImagAxis - minImag) / 5;
                int xPos = margin + i * (width - 2 * margin) / 5;
                int yPos = height - margin - i * (height - 2 * margin) / 5;
                g2.drawString(String.format(format, real), xPos - 10, yZero + 15);
                if (Math.abs(imag) > 1e-8) {
                    g2.drawString(String.format(format, imag), xZero - 30, yPos + 5);
                }
            }

            // Count infinite poles and zeros
            int infiniteZeros = 0;
            int infinitePoles = 0;
            for (String z : zeroStrings) {
                if (z.equals("infinity")) infiniteZeros++;
            }
            for (String p : poleStrings) {
                if (p.equals("infinity")) infinitePoles++;
            }

            // Draw poles (x) and zeros (o)
            g2.setColor(Color.BLUE);
            for (PolynomialRootFinder.Complex z : zeros) {
                int x = margin + (int) ((z.real - minReal) * (width - 2 * margin) / (maxRealAxis - minReal));
                int y = height - margin - (int) ((z.imag - minImag) * (height - 2 * margin) / (maxImagAxis - minImag));
                g2.drawOval(x - 5, y - 5, 10, 10);
            }
            g2.setColor(Color.RED);
            for (PolynomialRootFinder.Complex p : poles) {
                int x = margin + (int) ((p.real - minReal) * (width - 2 * margin) / (maxRealAxis - minReal));
                int y = height - margin - (int) ((p.imag - minImag) * (height - 2 * margin) / (maxImagAxis - minImag));
                g2.drawLine(x - 5, y - 5, x + 5, y + 5);
                g2.drawLine(x - 5, y + 5, x + 5, y - 5);
            }

            // Draw note for infinite poles/zeros
            g2.setColor(Color.WHITE);
            StringBuilder note = new StringBuilder();
            if (infiniteZeros > 0) {
                note.append(infiniteZeros).append(" zero").append(infiniteZeros > 1 ? "s" : "").append(" at ∞");
            }
            if (infinitePoles > 0) {
                if (infiniteZeros > 0) note.append(", ");
                note.append(infinitePoles).append(" pole").append(infinitePoles > 1 ? "s" : "").append(" at ∞");
            }
            if (note.length() > 0) {
                g2.drawString(note.toString(), margin, margin + 15);
            }

            if (poles.isEmpty() && zeros.isEmpty() && infinitePoles == 0 && infiniteZeros == 0) {
                g2.drawString("No poles or zeros to display", width / 2 - 80, height / 2);
            }
        }
    }
}